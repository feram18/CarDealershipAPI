...
public class StaffPanel extends JPanel implements GUIPanel {
	...
	// Arrays to populate sex, salary, and state comboboxes
	private final String[] sexOptions = { null, "F", "M" };

	private final String[] salaryOptions = { null, "< 10000", "< 20000", "< 30000", "< 40000", "< 50000", "< 60000",
			"< 70000", "< 80000", "< 90000", "< 100000", "< 110000", "< 120000", "> 120000" };
	...
	private JComboBox<String> inputSex;
	...
	public StaffPanel() {
		...
		menuItemEdit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					int choice = JOptionPane.showOptionDialog(null, inputFields, "Update Employee",
		...
		comboBoxSex.setBounds(116, 139, 86, 20);
		add(comboBoxSex);
		...
		comboBoxEmpType.setBounds(116, 170, 86, 20);
		add(comboBoxEmpType);
		...
		lblWorkLocation.setBounds(10, 204, 96, 15);
		add(lblWorkLocation);
		...
		comboBoxSalary.setBounds(116, 232, 86, 20);
		add(comboBoxSalary);
		...
		lblYearsWorked.setBounds(10, 266, 96, 15);
		add(lblYearsWorked);
		...
		textFieldYearsWorked.setBounds(116, 263, 86, 20);
		...
		btnAddEmployee.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					populateToAdd();
					if (JOptionPane.showOptionDialog(null, inputFields, "Add Employee", JOptionPane.DEFAULT_OPTION,
	...
	/***
	 * This method populates the comboboxes on staffTable with data retrieved from
	 * Employee table, which serve as a filter for the user and as a means of input
	 * validation.
	 */
	...
	public void populateComboBoxes() {
		...
		comboBoxWorkLoc.removeAllItems();
		...
		try {
			...
			query = "SELECT SSN FROM Employee WHERE employeeType=\"Manager\"";
			stmt = connection.prepareStatement(query);
			result = stmt.executeQuery();

			comboBoxManager.addItem(null);
			while (result.next()) {
				comboBoxManager.addItem(result.getString("SSN"));
			}

			/*
			 * Populates Manager's LastName, FirstName instead of SSN query =
			 * "SELECT DISTINCT fName, lName FROM Employee WHERE SSN = managerSSN ORDER BY lName;"
			 * ; stmt = connection.prepareStatement(query); result = stmt.executeQuery();
			 * 
			 * comboBoxManager.addItem(null); while(result.next()) {
			 * comboBoxManager.addItem(result.getString("lName") + ", " +
	 ...
	 * query with the parameters provided by the user. Results from Employee table
	 * are populated in the staffTable.
	 *
	...
	public void search() throws SQLException {
		try {
			...
			query = "SELECT SSN, " + "`fName` AS `First Name`, " + "`lName` AS `Last Name`, " + "`mInit` AS `M.I.`, "
					...
					+ "`managerSSN` AS `Manager SSN` " + "FROM Employee WHERE "; // Base query
			...
			if (comboBoxSex.getSelectedItem() != null) {
				addToQuery();
				query += "sex='" + comboBoxSex.getSelectedItem().toString() + "'";
			}
			...
			if (comboBoxManager.getSelectedItem() != null) {
				addToQuery();
				query += "SSN='" + comboBoxManager.getSelectedItem().toString() + "'";
			}
	...
	/**
	 * * This method makes the SQL query to add the a new row to the database's
	 * Employee table.
	 *
	 * @throws SQLException the SQL exception
	 */
	...
	/**
	 * * This method populates the fields of the Employee object with the current
	 * data to allow the user to edit the existing information.
	 *
	 * @throws SQLException the SQL exception
	 */
	...
	public void update() throws SQLException {
		try {
			...
			int selectedRow = staffTable.getSelectedRow();
			String SSN = (staffTable.getModel().getValueAt(selectedRow, 0)).toString();
			query = "UPDATE Employee SET " + "SSN='" + inputSSN.getText() + "', fName ='" + inputFirstName.getText()
					+ "', lName ='" + inputLastName.getText() + "', mInit ='" + inputMiddleInitial.getText()
					+ "', sex ='" + inputSex.getSelectedItem() + "', DOB ='" + inputDoB.getText() + "', phoneNo ='"
					+ inputPhoneNumber.getText() + "', employeeType ='" + inputWorkLocation.getSelectedItem()
					+ "', salary ='" + inputSalary.getText() + "', yearsWorked ='" + inputYearsWorked.getText()
					+ "', address ='" + inputAddress.getText() + "', hoursWorked ='" + inputHoursWorked.getText()
					+ "', username ='" + inputUsername.getText() + "', password ='" + inputPassword.getText()
					+ "', managerSSN ='" + inputManagerSSN.getSelectedItem() + "' WHERE SSN ='" + SSN + "';";
			PreparedStatement stmt = connection.prepareStatement(query);
	...
	/**
	 * * This method clears the input fields to avoid incorrect data on following
	 * edit attempt.
	 */
	...
	/**
	 * This method populates the comboboxes on the Add Employee popup window, as a
	...
	public void populateToAdd() throws SQLException {
		try {
			...
			inputSex = new JComboBox<String>(sexOptions);
